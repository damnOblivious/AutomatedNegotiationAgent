...
public class DummyAgent extends Agent {
	...
	/**
	 * Note: m{@link SimpleAgent} does not account for the discount factor in its
	 * computations
	 */
	private static double MINIMUM_BID_UTILITY = 0.0;
	private static double CURRENT_MIN_BID_UTILITY = 0.99;
	...
	public Action chooseAction() {
		...
		try {
			...
			if (actionOfPartner instanceof Offer) {
				...
				if (isAcceptable(offeredUtilFromOpponent, myOfferedUtil, time))   //TODO
	...
	private Bid getRandomBid() throws Exception {
		...
		} while (getUtility(bestBid) < CURRENT_MIN_BID_UTILITY);
	...
	double Paccept(double u, double t1) throws Exception {
		...
		System.out.println("Time-"+CURRENT_MIN_BID_UTILITY);
		if (u < 0 || u > 1.05)
		...
		if(u>=CURRENT_MIN_BID_UTILITY-0.0005)
			return 1;
		return 0;
		// if (t == 0.5)
		// 	return u;
